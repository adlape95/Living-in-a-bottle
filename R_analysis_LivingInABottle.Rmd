...
```{r}
ufc = read.csv2("./UFC-data/UFC-data.csv", sep = "\t")
head(ufc)
```
...
for (i in unique(ufc$Sample)) {
  aux = ufc[ufc$Sample == i,]
  plastic = aux[aux$CultureType == "Plastic", "UFC"]
  control = aux[aux$CultureType == "Control", "UFC"]
  res = c(res, wilcox.test(plastic, control)$p.value)
}

df = data.frame (unique(ufc$Sample), res)
...
```{r}
ufc = read.csv2("./UFC-data/UFC-data-v2.csv", sep = "\t")
head(ufc)
```
...
p = ggplot(data=ufc, aes(x=CultureType, y=UFCAvg, fill=CultureType)) + 
  facet_wrap( ~ Sample, scales = "free") +
  geom_bar(stat="identity", color="black", 
           position=position_dodge(), width = 0.8, alpha = 0.7) +
  geom_errorbar(aes(ymin=UFCAvg-SD, ymax=UFCAvg+SD), width=.2,
